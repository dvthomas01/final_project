...
EncoderVelocity encoders[NUM_ENCODERS] = {{ENCODER3_A_PIN, ENCODER3_B_PIN, CPR_312_RPM, 0.2}, 
...
void updatePIDs() {
    for (uint8_t i = 0; i < NUM_ENCODERS; i++) {
        ...
        controlEfforts[i] = pids[i].calculateParallel(velocities[i], setpoints[i + 2]); // +2 because encodered motors are 3, 4
